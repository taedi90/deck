package install

import (
	"context"
	"fmt"

	"github.com/taedi90/deck/internal/workflowexec"
)

type actionSelectorSpec struct {
	Action string `json:"action"`
}

func runFileAction(action string, spec map[string]any) error {
	switch action {
	case "download":
		return fmt.Errorf("file action download is not supported in apply dispatch without context")
	case "write":
		return runWriteFile(spec)
	case "copy":
		return runCopyFile(spec)
	case "edit":
		return runEditFile(spec)
	default:
		return fmt.Errorf("unsupported File action %q", action)
	}
}

func runPackages(ctx context.Context, spec map[string]any) error {
	action, err := decodeStepAction(spec)
	if err != nil {
		return fmt.Errorf("decode Packages action: %w", err)
	}
	switch action {
	case "download":
		return fmt.Errorf("packages action download is not supported in apply dispatch")
	case "install":
		return runInstallPackages(ctx, spec)
	default:
		return fmt.Errorf("unsupported Packages action %q", action)
	}
}

func runContainerd(ctx context.Context, spec map[string]any) error {
	return runContainerdConfig(ctx, spec)
}

func runRepository(ctx context.Context, spec map[string]any) error {
	action, err := decodeStepAction(spec)
	if err != nil {
		return fmt.Errorf("decode Repository action: %w", err)
	}
	switch action {
	case "configure":
		return runRepoConfig(ctx, spec)
	default:
		return fmt.Errorf("unsupported Repository action %q", action)
	}
}

func runImage(ctx context.Context, spec map[string]any) error {
	action, err := decodeStepAction(spec)
	if err != nil {
		return fmt.Errorf("decode Image action: %w", err)
	}
	switch action {
	case "download":
		return fmt.Errorf("image action download is not supported in apply dispatch")
	case "verify":
		return runVerifyImages(ctx, spec)
	default:
		return fmt.Errorf("unsupported Image action %q", action)
	}
}

func runKubeadm(ctx context.Context, spec map[string]any) error {
	action, err := decodeStepAction(spec)
	if err != nil {
		return fmt.Errorf("decode Kubeadm action: %w", err)
	}
	switch action {
	case "init":
		return runKubeadmInit(ctx, spec)
	case "join":
		return runKubeadmJoin(ctx, spec)
	case "reset":
		return runKubeadmReset(ctx, spec)
	default:
		return fmt.Errorf("unsupported Kubeadm action %q", action)
	}
}

func decodeStepAction(spec map[string]any) (string, error) {
	decoded, err := workflowexec.DecodeSpec[actionSelectorSpec](spec)
	if err != nil {
		return "", err
	}
	return decoded.Action, nil
}
