...
var toolMetadata = map[string]ToolMetadata{
	...
	"Command": {
		...
		FieldDocs: map[string]FieldDoc{
			...
			"spec.env":     {Description: "Additional environment variables passed to the command as key-value pairs.", Example: "{KUBECONFIG:/etc/kubernetes/admin.conf}"},
			"spec.sudo":    {Description: "Run the command with `sudo`. Defaults to `false`.", Example: "false"},
	...
	"Directory": {
		...
		FieldDocs: map[string]FieldDoc{
			...
			"spec.mode": {Description: "Directory permissions in octal notation. Applied on creation only.", Example: "0755"},
		},
	},
	...
	"File": {
		...
		FieldDocs: map[string]FieldDoc{
			...
			"spec.source":              {Description: "Download source descriptor. Required for `download`. Provide exactly one of `url`, `path`, or `bundle`.", Example: "{url:https://example.invalid/file.tar.gz}"},
			...
			"spec.output":              {Description: "Output target inside the bundle for the downloaded file. Required for `download`.", Example: "{path:files/bin/runc}"},
			"spec.output.path":         {Description: "Bundle-relative path where the downloaded file is written.", Example: "files/bin/runc"},
			...
			"spec.edits[].op":          {Description: "Edit operation type. `replace` substitutes the match; `append` adds `with` after each match. Defaults to `replace`.", Example: "replace"},
	...
	"Kubeadm": {
		...
		ActionNotes: map[string]string{
			...
			"join":  "`join` consumes a prepared join file and adds the node to an existing cluster.",
		...
		FieldDocs: map[string]FieldDoc{
			...
			"spec.mode":                  {Description: "`real` runs kubeadm normally. `stub` performs a dry-run contract check without executing kubeadm. Defaults to `real`.", Example: "real"},
			...
			"spec.joinFile":              {Description: "Path to the join command file produced by a prior `init` run. Required for `join`.", Example: "/tmp/deck/join.txt"},
			"spec.configFile":            {Description: "Path to an explicit kubeadm config file passed with `--config`. Use instead of individual flags when full config control is needed.", Example: "/tmp/deck/kubeadm.conf"},
			"spec.configTemplate":        {Description: "Named built-in config template rendered with the current vars. Use `default` for the deck-managed template.", Example: "default"},
			...
			"spec.asControlPlane":        {Description: "When `true`, joins the node as an additional control-plane member rather than a worker.", Example: "false"},
			"spec.force":                 {Description: "Pass `--force` to `kubeadm reset` to skip interactive confirmation prompts.", Example: "true"},
			"spec.ignoreErrors":          {Description: "Continue reset cleanup even when individual cleanup steps fail. Useful for nodes in a partially broken state.", Example: "true"},
	...
	"PackageCache": {
		...
		FieldDocs: map[string]FieldDoc{
			...
			"spec.restrictToRepos": {Description: "Limit the metadata update to these repository file paths or identifiers. Prevents fetching from online repos during an offline install.", Example: "[/etc/apt/sources.list.d/offline.list]"},
			"spec.excludeRepos":    {Description: "Repository identifiers to skip during the metadata update.", Example: "[updates]"},
		},
	},
	...
	"Packages": {
		...
		FieldDocs: map[string]FieldDoc{
			...
			"spec.restrictToRepos":       {Description: "Limit package manager visibility to these repository identifiers during the operation. Prevents accidental pulls from other configured repos.", Example: "[offline-kubernetes]"},
			"spec.excludeRepos":          {Description: "Repository identifiers to exclude from package resolution. Useful for suppressing online repos during an offline install.", Example: "[updates]"},
			...
			"spec.repo":                  {Description: "Package manager repository settings applied before `download`. Currently supports RPM module stream configuration.", Example: "{type:yum,modules:[...]}"},
			...
			"spec.backend":               {Description: "Container-based download backend for `download`. Enables package resolution inside a distro-specific container image when the host OS differs from the target.", Example: "{mode:container,runtime:docker,image:rockylinux:9}"},
		},
		Notes: []string{
	...
	"Repository": {
		...
		FieldDocs: map[string]FieldDoc{
			...
			"spec.format":          {Description: "Repository file format to write. `apt` produces a sources.list entry; `yum` produces a `.repo` file.", Example: "apt"},
			"spec.path":            {Description: "Explicit output path for the generated repository file. Defaults to a path derived from the first repository id when omitted.", Example: "/etc/apt/sources.list.d/offline.list"},
			...
			"spec.refreshCache":    {Description: "Optional package metadata refresh that runs after repository files are written. Equivalent to a follow-up `PackageCache` step.", Example: "{clean:true,update:true}"},
	...
	"Sysctl": {
		...
		FieldDocs: map[string]FieldDoc{
			"spec.writeFile": {Description: "Path to the sysctl drop-in file written with the given values. Must be under `/etc/sysctl.d/`.", Example: "/etc/sysctl.d/99-k8s.conf"},
			...
			"spec.apply":     {Description: "Run `sysctl --system` after writing the file to apply the values immediately without a reboot. Defaults to `false`.", Example: "true"},
		},
	},
	...
	"SystemdUnit": {
		...
		FieldDocs: map[string]FieldDoc{
			...
			"spec.contentFromTemplate": {Description: "Path to a template file relative to `workflows/` rendered with the current vars. Prefer this for parameterized unit files.", Example: "kubelet.service.tmpl"},
			...
			"spec.service.name":        {Description: "Service name to manage. Defaults to the unit file name when omitted.", Example: "kubelet"},
