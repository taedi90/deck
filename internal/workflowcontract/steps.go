...
func StepDefinitions() []StepDefinition {
	defs := []StepDefinition{
		...
		stepDefWithCategory("Command", "command.schema.json", "advanced", "advanced", "Run an explicit command as an escape hatch.", "Use this only when no typed step expresses the change clearly enough.", []string{"apply"}, nil),
