...
func StepDefinitions() []StepDefinition {
	defs := []StepDefinition{
		...
		stepDef("RunCommand", "command", "Command", "command", 10, "command.schema.json", "command", "public", "advanced", "Run an explicit command as an escape hatch.", "Use this only when no typed step expresses the change clearly enough.", []string{"apply"}, nil),
		...
		stepDef("ConfigureKernelModule", "kernel-module", "KernelModule", "kernel-module", 10, "kernel-module.schema.json", "kernel-module", "public", "system", "Load and persist kernel modules.", "Use this for modules that must be present before networking or container runtime setup.", []string{"apply"}, []string{"name", "names"}),
		...
		stepDef("ConfigureRepository", "repository", "Repository", "repository", 10, "repository.configure.schema.json", "repository.configure", "public", "packages", "Write apt or yum repository definitions.", "Use this before refreshing caches or installing packages from a local mirror.", []string{"apply"}, []string{"path"}),
		...
		stepDef("ConfigureSwap", "swap", "Swap", "swap", 10, "swap.schema.json", "swap", "public", "system", "Enable or disable swap and its persistence.", "Use this for Kubernetes-oriented host prep where swap policy matters.", []string{"apply"}, nil),
		...
		stepDef("ConfigureSysctl", "sysctl", "Sysctl", "sysctl", 10, "sysctl.schema.json", "sysctl", "public", "system", "Write and optionally apply sysctl values.", "Use this for kernel tunables that must survive reboot and may need immediate application.", []string{"apply"}, nil),
