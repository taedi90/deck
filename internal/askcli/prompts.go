...
func generationSystemPrompt(route askintent.Route, target askintent.Target, retrieval askretrieve.RetrievalResult) string {
	...
	b.WriteString("- Detailed topology, component/import guidance, vars guidance, and typed-step references are provided through retrieved context rendered from ask metadata.\n")
	b.WriteString("- Use Command only as an escape hatch when no typed step clearly matches the requested action.\n")
